...
string HdPackLoader::NormalizePackPath(string path)
{
...
{
	filename = NormalizePackPath(filename);
...
void HdPackLoader::ProcessPatchTag(vector<string> &tokens)
{
	...
	string patchPath = NormalizePackPath(tokens[0]);
	vector<uint8_t> fileData;
	if(!LoadFile(patchPath, fileData)) {
		checkConstraint(false, string("Patch file not found: " + patchPath));
	}

	std::transform(tokens[1].begin(), tokens[1].end(), tokens[1].begin(), ::toupper);
	if(_loadFromZip) {
		_data->PatchesByHash[tokens[1]] = VirtualFile(_hdPackFolder, patchPath);
	} else {
		_data->PatchesByHash[tokens[1]] = FolderUtilities::CombinePath(_hdPackFolder, patchPath);
	}
}
...
void HdPackLoader::ProcessBackgroundTag(vector<string> &tokens, vector<HdPackCondition*> conditions)
{
	...
	tokens[0] = NormalizePackPath(tokens[0]);
...
void HdPackLoader::ProcessBgmTag(vector<string> &tokens)
{
	...
	string filename = NormalizePackPath(tokens[2]);
