...
void BatteryManager::SaveBattery(string extension, uint8_t* data, uint32_t length)
{
	if(_romName.empty()) {
		...
		MessageManager::Log("[Battery] Skipping save: no ROM name");
		return;
	}

	_hasBattery = true;
	string path = GetBasePath(extension);
	errno = 0;
	ofstream out(path, ios::binary);
	if(!out) {
		MessageManager::Log("[Battery] Failed to open save file: " + path + " (" + std::strerror(errno) + ")");
		return;
	}
	...
	out.flush();
	if(out.good()) {
		MessageManager::Log("[Battery] Saved " + std::to_string(length) + " bytes to " + path);
	} else {
		MessageManager::Log("[Battery] Failed while writing save file: " + path + " (" + std::strerror(errno) + ")");
	}
}
