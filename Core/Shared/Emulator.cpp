...
#ifndef MESEN_NES_ONLY
#include "SNES/SnesConsole.h"
#include "SNES/SnesDefaultVideoFilter.h"
#endif
#include "NES/NesConsole.h"
#ifdef MESEN_NES_ONLY
...
void Emulator::Run()
{
	...
	while(!_stopFlag) {
		bool useRunAhead = _settings->GetEmulationConfig().RunAheadFrames > 0 && !IsDebugging() && !_audioPlayerHud && !_rewindManager->IsRewinding() && _settings->GetEmulationSpeed() > 0 && _settings->GetEmulationSpeed() <= 100;
		if(useRunAhead) {
		...
		if(_paused && !_stopFlag && !IsDebugging()) {
			WaitForPauseEnd();
		}
	}
...
void Emulator::WaitForPauseEnd()
{
	...
	while(_paused && !_rewindManager->IsRewinding() && !_stopFlag && !IsDebugging()) {
